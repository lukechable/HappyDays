...
export const setStatus = internalMutation({
  ...
  handler: async (ctx, { accountId, status }) => { await ctx.db.patch(accountId, { status, ...(status === "disconnected" ? { accessToken: undefined, accessTokenExpiresAt: undefined, watchExpiresAt: undefined, historyId: undefined, refreshTokenEnc: "" } : {}) }); },
});
