    ...
    <main className="min-h-svh bg-background">
      ...
      <div className="mx-auto max-w-3xl px-6 py-8">
        ...
        {options.error ? <p className="text-sm text-error">{options.error}</p> : !options.data ? <div className="space-y-2">{[0, 1, 2].map((i) => <div key={i} className="h-16 animate-pulse rounded-xl bg-muted" />)}</div> : options.data.appointmentTypes.length === 0 ? <p className="text-sm text-fg-tertiary">Online booking isn’t open at the moment. Please call the practice.</p> : (
          <>
            {step === 1 && <ul className="grid gap-3 sm:grid-cols-2">{options.data.appointmentTypes.map((t) => <li key={t.id}><button type="button" onClick={() => { setTypeId(t.id); if (options.data!.practitioners.length === 1) setPracId(options.data!.practitioners[0].id); }} className="hd-card w-full rounded-2xl bg-card p-4 text-left ring-1 ring-black/[0.06] dark:ring-white/10"><div className="font-display text-lg">{t.name}</div><div className="mt-1 text-sm text-fg-secondary">{t.durationMinutes} minutes{t.telehealth ? " · telehealth available" : ""}</div>{t.description && <p className="mt-2 text-sm text-fg-tertiary">{t.description}</p>}<div className="num mt-3 text-sm"><b>{aud(t.payNowCents)}</b> {t.mode === "deposit" ? `deposit now, ${aud(t.feeCents)} total` : "paid now"}</div></button></li>)}</ul>}
            {step === 2 && <div><Back onClick={() => setTypeId(null)} /><ul className="grid gap-3 sm:grid-cols-2">{options.data.practitioners.map((p) => <li key={p.id}><button type="button" onClick={() => setPracId(p.id)} className="hd-card w-full rounded-2xl bg-card p-4 text-left ring-1 ring-black/[0.06] dark:ring-white/10"><div className="font-display text-lg">{p.name}</div>{p.designation && <div className="text-sm text-fg-secondary">{p.designation}</div>}{p.description && <p className="mt-2 text-sm text-fg-tertiary">{p.description}</p>}</button></li>)}</ul></div>}
            {step === 3 && (
              <div>
