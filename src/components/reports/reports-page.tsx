...
export function ReportsPage({ kind }: { kind: ReportKind }) {
  ...
  const upload = async (list: FileList | File[], replaces?: Id<"files">) => {
    ...
    toast.success(arr.length === 1 ? "Report uploaded" : `${arr.length} reports uploaded`);
  };

  const all = reports ?? [];
  const rows = all.filter((r) => (!matterFilter || r.matterId === matterFilter) && (!onlyPending || !r.matter?.deliveredAt) && (!q.trim() || [r.name, r.matter?.name, r.uploadedByName].some((v) => v?.toLowerCase().includes(q.trim().toLowerCase()))));
  ...
  return (
    <div className="space-y-5" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); if (e.dataTransfer.files.length) void upload(e.dataTransfer.files); }}>
      ...
      <div className="grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        <Kpi label={copy.title} value={reports ? all.length : "…"} sub="latest versions" />
        <Kpi label="Delivered" value={reports ? delivered : "…"} tone={delivered ? "good" : undefined} sub="matter marked delivered" />
        <Kpi label="Awaiting delivery" value={reports ? awaiting : "…"} tone={awaiting ? "warn" : undefined} sub="on an open matter" />
        <Kpi label="No matter" value={reports ? unlinked : "…"} sub="link one from the row" />
      </div>
      <Panel>
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <button type="button" onClick={() => setOnlyPending(false)} className={cn("rounded-full px-2.5 py-1 text-xs", !onlyPending ? "bg-foreground text-background" : "bg-muted text-fg-secondary hover:text-foreground")}>All</button>
          <button type="button" onClick={() => setOnlyPending(true)} className={cn("rounded-full px-2.5 py-1 text-xs", onlyPending ? "bg-foreground text-background" : "bg-muted text-fg-secondary hover:text-foreground")}>Not yet delivered</button>
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search report, matter, who" className="ml-auto h-8 w-56" />
          <select value={matterFilter ?? ""} onChange={(e) => replaceUrl(`${base}${e.target.value ? `?matter=${e.target.value}` : ""}`)} className="h-8 rounded-lg border border-input bg-card px-2 text-xs" aria-label="Matter"><option value="">All matters</option>{(matters ?? []).map((m) => <option key={m._id} value={m._id}>{m.name}</option>)}</select>
        </div>
            ...
            {rows.map((r) => (
              <tr key={r._id} className="group hover:bg-muted/50">
                ...
                <td><div className="flex justify-end gap-0.5 opacity-60 group-hover:opacity-100">
                  ...
                  <RowAction label="Download codes" href={`/files?tab=codes`} icon={<KeyRound className="size-3.5" />} />
