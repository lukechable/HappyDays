...
export function FolderList({ view, labelId, labels, badges, onSelect, onPrepare, onLabelsChanged, onDropThreads }: { view: ViewKey; labelId?: string; labels: Label[] | undefined; badges: { overdue: number; assigned: number }; onSelect: (view: ViewKey, labelId?: string) => void; onPrepare?: (view: ViewKey, labelId?: string) => void; onLabelsChanged: () => void; onDropThreads?: (target: DropTarget, ids: string[]) => void }) {
              ...
              return (
                <li key={l.id} className="group" {...dropProps(l.id, { labelId: l.id })}>
                  <div className={cn("hd-row flex items-center gap-1 rounded-lg pr-1", active ? "bg-foreground text-background" : "text-fg-secondary hover:bg-muted hover:text-foreground", isOver && "ring-2 ring-blue ring-offset-1 ring-offset-surface-2 bg-blue-soft text-foreground")} style={{ paddingLeft: depth * 10 }}>
                    ...
                    <button type="button" onPointerEnter={() => onPrepare?.("label", l.id)} onFocus={() => onPrepare?.("label", l.id)} onTouchStart={() => onPrepare?.("label", l.id)} onClick={() => onSelect("label", l.id)} className="flex min-w-0 flex-1 items-center gap-2 px-2 py-[6px] text-[13px] leading-tight"><Folder className="size-3.5 shrink-0" style={{ color: active ? undefined : l.color?.backgroundColor }} /><span className="min-w-0 flex-1 truncate text-left">{short}</span>{l.unread > 0 && <span className={cn("num text-[10.5px] font-semibold", active ? "text-background/80" : "text-fg-tertiary")}>{l.unread}</span>}</button>
