...
export function SignOutButton({ className, children }: { className?: string; children: ReactNode }) {
  ...
  if (mode === "guest") return <form method="POST" action="/api/guest/logout" className="contents"><button type="submit" className={className} aria-label="Sign out" title="Sign out">{children}</button></form>;
...
function ClerkSignOut({ className, children }: { className?: string; children: ReactNode }) {
  ...
  return <button type="button" onClick={() => void signOut({ redirectUrl: "/signin" })} className={className} aria-label="Sign out" title="Sign out">{children}</button>;
}
